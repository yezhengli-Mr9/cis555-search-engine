...
public class DistributedCluster implements Runnable {
	...
	static AtomicBoolean quit = new AtomicBoolean(true);
	...
	ExecutorService executor = new ThreadPoolExecutor(6, 6, 0L, TimeUnit.MILLISECONDS,
	...
	public TopologyContext submitTopology(String name, Config config, 
		...
		if (!spoutStreams.isEmpty())
			closeSpoutInstances();
		if (!boltStreams.isEmpty())
			closeBoltInstances();
		...
		boltStreams = new HashMap<>();
		spoutStreams = new HashMap<>();
		streams = new HashMap<>();
		...
		// scheduleSpouts(); Moving this to run() method below so that tasks are not queued until job starts running
		...
		for (String stream: topo.getBolts().keySet()) {
			...
			for (String worker: WorkerHelper.getWorkers(config)) {
				...
				if (workerId++ != Integer.valueOf(config.get("workerIndex"))) {
					...
					for (int i = 0; i < count; i++) {
						...
						//System.out.println("Adding a remote route from " + stream + " to " + worker);
					}
				...
				// Create one local executor for each node for us!
					...
					for (IRichBolt bolt: boltStreams.get(stream)) {
						...
						//System.out.println("Adding a route from " + decl.getStream() + " to " + bolt);
					}
				}
			}
			...
			if (topo.getBolts().containsKey(decl.getStream())) {
				for (IRichBolt bolt: boltStreams.get(decl.getStream())) {
					...
					//System.out.println(decl.getStream()+" router set");
				}
			} else {
